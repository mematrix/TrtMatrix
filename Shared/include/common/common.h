...
inline void setDummyInt8Scales(const IBuilderConfig* c, INetworkDefinition* n)
{
    ...
    if (c->getFlag(BuilderFlag::kINT8))
    {
        ...
        gLogger.log(Logger::Severity::kWARNING, "Int8 calibrator not provided. Generating dummy per tensor scales. Int8 accuracy is not guaranteed.");
...
inline void loadLibrary(const std::string& path)
{
    ...
    if (handle == nullptr)
    {
        ...
        gLogger.log(Logger::Severity::kERROR, ("Could not load plugin library: " + path).c_str());
#else
